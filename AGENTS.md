...
- **Current-bubble custom settings**: per-user overrides stored on `user_current_bubble` (`custom_color`, `custom_text_color`, `custom_font_family`, `custom_text`, `custom_size`; all nullable; migrations 006/007). Profile.vue「气泡自定义设置」card edits them via `GET/PUT /api/bubbles/current-settings` (PUT requires a selected bubble → 400 otherwise; empty string = clear to NULL). `GET /api/bubbles/get-bubble` applies them via `svg_util.apply_customizations()`: non-empty values fill `{c}`/`{t}`/`{n}` (XML-escaped) and set `font-family` on all `<text>` tags; unset items keep placeholders for downstream replacement. Templates **without** `{c}`/`{t}` (hardcoded colors like `fill="#030000"`) are mapped heuristically (mirrors frontend `autoMapColors`): `<text>` fill → `custom_text_color`, first other fill/stroke → `custom_color`, replacing all occurrences of that value. `custom_size` rewrites text tag case: `large` → `<TEXT>`, `small` → `<text>`, empty → unchanged. JS mirror: `applyCustomizations()` in `svgHelper.js` (used by Profile preview). Fallback to official bubble has no customizations.
